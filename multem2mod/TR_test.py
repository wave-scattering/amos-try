...
def eval(i):
    create_input(npts, ak1[i], ak2, zinf, zsup, polar, lmax, r_ratio)
...
if __name__ == "__main__":
    ...
    #-------------------------------- input file for Zarina Sadrieva(2019) ---------------------------------
    # from_theta_rad = np.arcsin(0.0)
    # to_theta_rad = np.arcsin(1.0)
    ...
    lambda_incident = 750
    a = 475
    k = 2*np.pi/lambda_incident/a

    theta = np.linspace(from_theta_deg, to_theta_deg, n_theta)
    ...
    # plt.plot(ak1, data_arr[2], label='refl', lw=2.0)
