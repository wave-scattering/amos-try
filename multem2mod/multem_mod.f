...
!     IMPLICIT NONE
!     ------------------------------------------------------------------
!     ONECHK IS A COMPANION ROUTINE OF  ERRCHK.  IT IS CALLED JUST LIKE
!     ERRCHK, AND MESSAGES FROM IT MAY BE  SUPPRESSED BY AN APPROPRIATE
!     CALL TO ERXSET.IT DIFFERS FROM ERRCHK IN THAT EACH CALL TO ONECHK
!     WILL PRODUCE NO MORE THAN ONE  PRINTED MESSAGE, REGARDLESS OF HOW
!     MANY TIMES THAT CALL IS  EXECUTED,   AND ONECHK NEVER  TERMINATES
!     EXECUTION.  ITS  PURPOSE  IS TO PROVIDE ONE-TIME-ONLY INFORMATIVE
!     DIAGNOSTICS.
!
...
!    & XXMAT2((LMAX+1)**2-1,(LMAX+1)**2-1)
      complex(dp) vXXMAT1((LMAX+1)**2-1,(LMAX+1)**2-1),
