c$$$ADIMMULTEM.  HETEROSTRUCTURES OF PHOTONIC CRYSTALS: FREQUENCY BANDS AND  
c$$$1   TRANSMISSION COEFFICIENTS.  N. STEFANOU, V. YANNOPAPAS, A. MODINOS.
c$$$REF. IN COMP. PHYS. COMMUN. 113 (1998) 49
c$$$CCCCCCCCC-------> THIS IS THE FIRST LINE OF THE FILE
c$$$CCCCCCCCC-------> HERE STARTS THE FORTRAN SOURCE CODE
      PROGRAM MULTEM
      IMPLICIT NONE 
C     ------------------------------------------------------------------ 
C     A B S T R A C T  
...
C                   THE  INTERFACES OF THE STRUCTURE:  AK(1) AND AK(2)  
C                   (AND THE  FREQUENCY). THE PROGRAM  CALCULATES  THE  
C                   TRANSMISSION, REFLECTION,  ABSORPTION COEFFICIENTS 
C                   OF A FINITE SLAB 
...
C                   INTERFACES OF THE STRUCTURE: AK(1) AND AK(2) 
...
C     AK(1),AK(2) : WAVEVECTOR COMPONENTS PARALLEL  TO THE  INTERFACES  
...
C 
      ...
      REAL*8     S(NCOMPD,NPLAND),AL(3),D(NCOMPD) 
      ...
      COMPLEX*16 EINCID(IGKD),EIN(3),EPS2(NCOMPD),EPS3(NCOMPD) 
...
C 
      ...
      DATA CZERO/(0.D0,0.D0)/,EINCID/IGKD*(0.D0,0.D0)/
...
C     
     ...
     &				   STOP 'ILLEGAL INPUT VALUE OF NCOMP' 
      ...
      IF(NP.LE.1)                  STOP 'ILLEGAL INPUT VALUE OF  NP '
              IF(KTYPE.GE.2) THEN 
      READ(10,204) AK(1),AK(2),POLAR,FEIN 
      AK(1)=2.D0*PI*AK(1) 
      AK(2)=2.D0*PI*AK(2)
      IF(KTYPE.LT.3) THEN 
      WRITE(6,222)
      ...
      WRITE(6,223)
      ENDIF
      IF(KTYPE.EQ.2) WRITE(6,207) AK(1),AK(2),POLAR 
      IF(KTYPE.EQ.3) WRITE(6,225) AK(1),AK(2)
                             ELSE 
     ...
     &             MU3(ICOMP),EPS3(ICOMP)
      IF(DIMAG(MU1(ICOMP)).NE.0.D0.OR.DIMAG(EPS1(ICOMP)).NE.0.D0.
     &OR.DIMAG(MU3(ICOMP)).NE.0.D0.OR.DIMAG(EPS3(ICOMP)).NE.0.D0) 
     &               STOP 'ILLEGAL INPUT: ABSORBING HOST MEDIUM'            
    ...
    3 CONTINUE 
      ...
      IF(KEMB.EQ.1) THEN 
      READ(10,205) MUEMB,EPSEMB 
      IF(DIMAG(MUEMB).NE.0.D0.OR.DIMAG(EPSEMB).NE.0.D0)
     &          STOP 'ILLEGAL INPUT: ABSORBING EMBEDDING MEDIUM'            
      D1=SQRT(MUEMB*EPSEMB) 
		    ENDIF 
		     ELSE
...
C 
    ...
    5 CONTINUE 
      ...
      GO TO 20 
                        END IF 
      IF(POLAR.EQ.'P ') THEN 
      EIN(1)=AKZIN*AK(1)/KAPIN/AKXY 
      EIN(2)=AKZIN*AK(2)/KAPIN/AKXY 
      EIN(3)=-AKXY/KAPIN 
                        ELSE 
      EIN(1)=DCMPLX(-AK(2)/AKXY,0.D0) 
      EIN(2)=DCMPLX( AK(1)/AKXY,0.D0) 
      EIN(3)=CZERO 
                        END IF 
   20 CONTINUE 
      CALL REDUCE(AR1,AR2,AK,IGMAX,G,IG0,EMACH)   !"AK" IN SBZ******* 
      DO 2 I=1,3 
      EINCID(3*IG0-3+I)=EIN(I) 
    2 CONTINUE 
                     ELSE
      CALL REDUCE(AR1,AR2,AK,IGMAX,G,IG0,EMACH)   !"AK" IN SBZ******* 
		     ENDIF    
...
C
   ...
   30        CONTINUE 
             IF(KEMB.EQ.1) THEN
             CALL HOSLAB(IGMAX,KAPR,(KAPR+KAPIN)/2.D0,KAPIN,AK,G,0.D0, 
     &                   0.D0,0.D0,QIR,QIIR,QIIIR,QIVR,EMACH) 
	     CALL PAIR(IGKMAX,QIL,QIIL,QIIIL,QIVL,QIR,QIIR,QIIIR,QIVR) 
	     DO 11 IGK1=1,IGKMAX 
	     DO 11 IGK2=1,IGKMAX 
   ...
   11        CONTINUE 
             CALL HOSLAB(IGMAX,KAPIN,(KAPL+KAPIN)/2.D0,KAPL,AK,G,0.D0, 
     &                   0.D0,0.D0,QIL,QIIL,QIIIL,QIVL,EMACH) 
	     CALL PAIR(IGKMAX,QIL,QIIL,QIIIL,QIVL,QIR,QIIR,QIIIR,QIVR) 
					  ENDIF
      CALL SCAT(IGMAX,ZVAL,AK,G,DREAL(KAPIN),EINCID,QIL,QIIIL) 
                     ELSE 
C 
...
C 
  ...
  225 FORMAT(3X,'K_PARALLEL=',2F12.6) 
      END 
C======================================================================= 
      SUBROUTINE SCAT(IGMAX,ZVAL,AK,G,KAPIN,EINCID,QI,QIII)
      IMPLICIT NONE
C     ------------------------------------------------------------------ 
...
C 
      ...
      REAL*8     DOWN,REFLE,TRANS,ABSOR,GKZ,TES1 
      COMPLEX*16 CZERO 
C 
C ..  LOCAL ARRAYS  .. 
C 
...
C     ------------------------------------------------------------------ 
     ...
     &            G(2,IG1)) 
      GKZ=0.D0 
      IF((KAPIN*KAPIN-TES1).GT.0.D0) GKZ=SQRT(KAPIN*KAPIN-TES1)
      DO 1 K1=1,3 
      IGK1=IGK1+1 
      ETRANS(IGK1)=CZERO 
      EREFLE(IGK1)=CZERO 
    ...
    2 CONTINUE 
C 
      DOWN =DOWN +EINCID(IGK1)*CONJG(EINCID(IGK1))*GKZ 
      TRANS=TRANS+ETRANS(IGK1)*CONJG(ETRANS(IGK1))*GKZ 
      REFLE=REFLE+EREFLE(IGK1)*CONJG(EREFLE(IGK1))*GKZ 
    1 CONTINUE
      TRANS=TRANS/DOWN 
      REFLE=REFLE/DOWN 
...
C 
 ...
 10   CONTINUE
    ...
    5 CONTINUE 
      DO 6 J=1,4 
      PP(J,1,1)=P(J,1)*CFI*CFI+P(J,2)*SFI*SFI 
      PP(J,1,2)=(P(J,1)-P(J,2))*SFI*CFI 
      PP(J,2,1)=PP(J,1,2) 
      PP(J,2,2)=P(J,1)*SFI*SFI+P(J,2)*CFI*CFI 
      PP(J,3,3)=P(J,3) 
    6 CONTINUE 
     ...
     &	             GKKZ1*DL(3)-GKKZ3*DR(3))) 
      ...
      DO 8 JB=1,3 
      IB=3*IG1-3+JB 
      QI  (IA,IB)=CQI  *PP(1,JA,JB) 
      QII (IA,IB)=CQII *PP(2,JA,JB) 
      QIII(IA,IB)=CQIII*PP(3,JA,JB) 
      QIV (IA,IB)=CQIV *PP(4,JA,JB) 
    8 CONTINUE 
    7 CONTINUE 
    3 CONTINUE 
      RETURN
...
C ..  ARRAY ARGUMENTS  .. 
C 
      COMPLEX*16 DLME(3,LM1SQD),DLMH(3,LM1SQD),GK(3) 
C 
C  .. LOCAL SCALARS .. 
C 
      INTEGER    K,II,L,M,I 
...
C 
    ...
    1 DLMH(K,1)=CZERO
      ...
      I=L*L+L+M+1 
      Z3=YLM(I) 
      DLMH(1,II)=COEF*(ALPHA*Z1+BETA*Z2) 
      DLMH(2,II)=-CI*COEF*(ALPHA*Z1-BETA*Z2) 
      DLMH(3,II)=DFLOAT(M)*Z3*COEF 
      DLME(1,II)=-(GK(2)*DLMH(3,II)-GK(3)*DLMH(2,II))/KAPPA
      DLME(2,II)=-(GK(3)*DLMH(1,II)-GK(1)*DLMH(3,II))/KAPPA
      DLME(3,II)=-(GK(1)*DLMH(2,II)-GK(2)*DLMH(1,II))/KAPPA
    2 CONTINUE 
      RETURN 
...
C ..  ARRAY ARGUMENTS  .. 
C 
      COMPLEX*16 AE(3,LM1SQD),AH(3,LM1SQD),GK(3) 
C 
C ..  LOCAL SCALARS  .. 
C 
      INTEGER    M,II,L,I,K 
...
C 
    ...
    3 AH(K,1)=CZERO 
      ...
      AE(1,II)= SIGNUS*(Z1*CI*N3-Z2*CI*N3+Z3*N2) 
      AE(2,II)=-SIGNUS*(Z1*N3+Z2*N3+Z3*N1) 
      AE(3,II)= SIGNUS*(Z1*(N2-CI*N1)+Z2*(N2+CI*N1)) 
      AH(1,II)= SIGNUS*(Z1+Z2) 
      AH(2,II)= SIGNUS*CI*(Z1-Z2) 
      AH(3,II)=-SIGNUS*Z3 
    2 CONTINUE 
    1 CONTINUE 
      RETURN 
...
C 
      INTEGER    LMAX 
      REAL*8     RAP 
      COMPLEX*16 EPSSPH,EPSMED,MUSPH,MUMED 
C 
C ..  ARRAY ARGUMENTS  .. 
C 
...
C 
      ...
      PARAMETER (LMAXD=7,LMAX4D=4*LMAXD+1) 
C 
C ..  SCALAR ARGUMENTS  .. 
C 
...
C 
    ...
    4 CONTINUE 
      ...
      DO 5 IGK2=1,IGKMAX 
      W1(IGK1,IGK2)=CZERO 
      W2(IGK1,IGK2)=QIIR (IGK1,IGK2) 
      W3(IGK1,IGK2)=QIIIL(IGK1,IGK2) 
      W4(IGK1,IGK2)=CZERO 
      DO 6 IGK3=1,IGKMAX 
      W1(IGK1,IGK2)=W1(IGK1,IGK2)+QIR (IGK1,IGK3)*QINV1(IGK3,IGK2) 
      W4(IGK1,IGK2)=W4(IGK1,IGK2)+QIVL(IGK1,IGK3)*QINV2(IGK3,IGK2) 
      DO 7 IGK4=1,IGKMAX 
      W2(IGK1,IGK2)=W2(IGK1,IGK2)+QIR(IGK1,IGK3)* 
     &               QIIL(IGK3,IGK4)*QINV2(IGK4,IGK2) 
      W3(IGK1,IGK2)=W3(IGK1,IGK2)+QIVL(IGK1,IGK3)* 
     &               QIIIR(IGK3,IGK4)*QINV1(IGK4,IGK2) 
    7 CONTINUE 
    6 CONTINUE 
    5 CONTINUE 
      DO 8 IGK1=1,IGKMAX 
      ...
      QIIL (IGK1,IGK2)=W2(IGK1,IGK2) 
      QIIIL(IGK1,IGK2)=W3(IGK1,IGK2) 
...
C 
      ...
      PARAMETER (LMAXD=7,LMAX1D=LMAXD+1,LMODD=(LMAXD*LMAX1D)/2) 
      PARAMETER (LM1SQD=LMAX1D*LMAX1D,IGD=21,IGKD=3*IGD,NELMD=7680) 
...
C 
      INTEGER    LMAX,IGMAX 
      REAL*8     RAP,A0,EMACH  
      COMPLEX*16 EPSMED,EPSSPH,MUMED,MUSPH,KAPPA 
...
C 
      ...
      COMPLEX*16 AE(3,LM1SQD),AH(3,LM1SQD),GKK(3,IGD) 
      COMPLEX*16 GK(3),LAME(3),LAMH(3) 
      COMPLEX*16 XEVEN(LMEVEN,LMEVEN),XODD(LMODD,LMODD) 
      COMPLEX*16 TE(LMAX1D),TH(LMAX1D),BMEL1(LMTD),BMEL2(LMTD) 
      COMPLEX*16 XXMAT1(LMTD,LMTD),XXMAT2(LMTD,LMTD) 
      COMPLEX*16 DLME(3,LM1SQD),DLMH(3,LM1SQD)
C 
C ..  COMMON BLOCKS .. 
C 
      REAL*8     AR1(2),AR2(2)  
...
C 
    ...
    8 CONTINUE 
                      ...
                      IGK1=IGK1+1 
                      SIGNUS=1.D0 
                      IF(K2.EQ.3.AND.K1.NE.3) SIGNUS=-1.D0 
                      IF(K1.EQ.3.AND.K2.NE.3) SIGNUS=-1.D0 
...
C 
      ...
      PARAMETER(IGD=21,IGKD=3*IGD,IGK2D=2*IGKD) 
C 
C ..  SCALAR ARGUMENTS  .. 
C 
      INTEGER    IGMAX 
...
C 
      ...
      INTEGER    JID,KD,LIB1,LIB2,LU,LP,LN,IDI,IFAIL,IGK3,IGK2M 
      REAL*8     PI,PRODT,AKA,BKZRE,BKZIM 
...
C 
      ...
      COMPLEX*16 QH1(IGKD,IGKD),QH2(IGKD,IGKD),AKZ(IGK2D),GKKR(3,IGD) 
      COMPLEX*16 COMVEC(IGK2D,IGK2D),PRODP(IGD),PRODN(IGD),GKKL(3,IGD)
C 
C ..  INTRINSIC FUNCTIONS .. 
C 
...
C*****'MACHEP' OF THE SUBROUTINE COMLR2 IS CHOSEN GREATER THAN 2**(-47) 
    ...
    2 CONTINUE 
C 
C****** EXCLUDE SOLUTIONS WHICH DO NOT SATISFY GAUSS LAW ****** 
C 
      LU=1
      LP=1
      LN=1
      ...
      PRODP(IDI)=CZERO 
      PRODN(IDI)=CZERO 
      GKKL(1,IDI)=DCMPLX((AK(1)+G(1,IDI)),0.D0) 
      GKKL(2,IDI)=DCMPLX((AK(2)+G(2,IDI)),0.D0) 
      GKKL(3,IDI)=SQRT(KAPL*KAPL-GKKL(1,IDI)*GKKL(1,IDI)- 
     &                            GKKL(2,IDI)*GKKL(2,IDI)) 
      GKKR(1,IDI)= GKKL(1,IDI) 
      GKKR(2,IDI)= GKKL(2,IDI) 
      GKKR(3,IDI)=-SQRT(KAPR*KAPR-GKKR(1,IDI)*GKKR(1,IDI)- 
     &                            GKKR(2,IDI)*GKKR(2,IDI)) 
      DO 3 JID=1,3 
      PRODP(IDI)=PRODP(IDI)+GKKL(JID,IDI)*COMVEC(3*IDI-3+JID,KD) 
      PRODN(IDI)=PRODN(IDI)+GKKR(JID,IDI)*COMVEC(3*(IDI+IGMAX)-3+JID,KD) 
    3 CONTINUE 
      PRODT=PRODT+ABS(PRODP(IDI))+ABS(PRODN(IDI)) 
    4 CONTINUE 
C*****WARNING!! THE APPROPRIATE LIMITS FOR PRODT AND DIMAG(AKZ(KD)) 
C*****DEPEND STRONGLY ON IGMAX. 
      IF(PRODT.GT.1.0D-2) GO TO 10
   ...
   6  CONTINUE 
    ...
    8       CONTINUE 
     ...
     &RECIPROCAL LATTICE VECTORS ' 
      GOTO 6
      END 
c$$$CCCCCCCCC-------> HERE ENDS THE FORTRAN SOURCE CODE
c$$$CCCCCCCCC-------> HERE STARTS THE FIRST INPUT DATA FILE (READ FROM UNIT 10)
c$$$                   ********************************************
c$$$          	   ********INPUT FILE FOR TRANSMISSION*********
c$$$		   ********************************************
c$$$   KTYPE = 2   KSCAN = 1   KEMB  = 1    LMAX = 4   NCOMP = 4   NUNIT = 6
c$$$ ALPHA =    1.000000  BETA =    1.000000   FAB =   90.000000  RMAX =   16.000000
c$$$  NP =  17  ZINF =   1.00000000  ZSUP =   2.000000000
c$$$  THETA/AK(1) =   0.00000000     FI/AK(2) =   0.00000000   POLAR =S     FEIN =   0.00
c$$$
c$$$Give information for the "NCOMP" components 
c$$$
c$$$     IT  = 2
c$$$     MUMED =   1.00000000   0.00000000     EPSMED=   2.00000000   0.00000000
c$$$   NPLAN = 2  NLAYER = 1
c$$$       S =   0.35000000     MUSPH =   1.00000000   0.00000000     EPSSPH=   5.00000000   0.00000000
c$$$xyzDL 0.0  0.0  0.4
c$$$xyzDR 0.25 0.25 0.3
c$$$       S =   0.35000000     MUSPH =   1.00000000   0.00000000     EPSSPH=   5.00000000   0.00000000
c$$$xyzDL 0.25 0.25 0.3
c$$$xyzDR 0.25 0.25 0.4
c$$$     IT  = 1
c$$$  DSLAB       =   0.00000000    
c$$$     MU1   =   1.00000000   0.00000000     EPS1  =   2.00000000   0.00000000
c$$$     MU2   =   1.00000000   0.00000000     EPS2  =   1.10000000   0.00000000
c$$$     MU3   =   1.00000000   0.00000000     EPS3  =   7.00000000   0.00000000
c$$$xyzDL 0.0  0.0  0.0
c$$$xyzDR 0.0  0.0  0.0
c$$$     IT  = 2
c$$$     MUMED =   1.00000000   0.00000000     EPSMED=   7.00000000   0.00000000
c$$$   NPLAN = 1  NLAYER = 1
c$$$       S =   0.25000000     MUSPH =   1.00000000   0.00000000     EPSSPH=   5.00000000   0.00000000
c$$$xyzDL 0.25 0.25 0.3
c$$$xyzDR 0.0  0.0  0.3
c$$$     IT  = 1
c$$$  DSLAB       =   1.00000000    
c$$$     MU1   =   1.00000000   0.00000000     EPS1  =   7.00000000   0.00000000
c$$$     MU2   =   1.00000000   0.00000000     EPS2  =   9.00000000   0.00000000
c$$$     MU3   =   1.00000000   0.00000000     EPS3  =   2.00000000   0.00000000
c$$$xyzDL 0.0  0.0  0.0
c$$$xyzDR 0.0  0.0  0.0
c$$$     MUEMB =   1.00000000   0.00000000    EPSEMB =   1.00000000   0.00000000
c$$$CCCCCCCCC-------> HERE ENDS THE FIRST INPUT DATA FILE
c$$$CCCCCCCCC-------> HERE STARTS THE SECOND INPUT DATA FILE (READ FROM UNIT 10)
c$$$                   ********************************************
c$$$          	   *******INPUT FILE FOR BAND STRUCTURE********
c$$$		   ********************************************
c$$$   KTYPE = 3   KSCAN = 1   KEMB  = 1    LMAX = 4   NCOMP = 4   NUNIT = 6
c$$$ ALPHA =    1.000000  BETA =    1.000000   FAB =   90.000000  RMAX =   16.000000
c$$$  NP =  17  ZINF =   1.00000000  ZSUP =   2.000000000
c$$$  THETA/AK(1) =   0.00000000     FI/AK(2) =   0.00000000   POLAR =S     FEIN =   0.00
c$$$
...
c$$$xyzDR 0.0  0.0  0.0
c$$$xyzAL 0.0  0.0  3.0
c$$$CCCCCCCCC-------> HERE ENDS THE SECOND INPUT DATA FILE
c$$$CCCCCCCCC-------> HERE STARTS THE FIRST OUTPUT FILE (WRITTEN IN UNIT 6)
c$$$     ****************************************************
c$$$     *** OUTPUT: TRANSMITTANCE/REFLECTANCE/ABSORBANCE ***
c$$$     ****************************************************
c$$$   K_PARALLEL=     .000000     .000000     S POLARIZATION
c$$$   COMPONENT NR. 1   TYPE:  PHOTONIC CRYSTAL 
c$$$   MU :   1.00000    .00000 |       1.00000    .00000      1.00000    .00000   
c$$$   EPS:   2.00000    .00000 |       5.00000    .00000      5.00000    .00000   
c$$$     S:                              .35000                 .35000             
c$$$                                  1 UNIT LAYERS
c$$$   COMPONENT NR. 2   TYPE:  HOMOGENEOUS PLATE
c$$$   MU :   1.00000    .00000 |    1.00000    .00000 |    1.00000    .00000
c$$$   EPS:   2.00000    .00000 |    1.10000    .00000 |    7.00000    .00000
c$$$   COMPONENT NR. 3   TYPE:  PHOTONIC CRYSTAL 
c$$$   MU :   1.00000    .00000 |       1.00000    .00000   
c$$$   EPS:   7.00000    .00000 |       5.00000    .00000   
c$$$     S:                              .25000             
c$$$                                  1 UNIT LAYERS
c$$$   COMPONENT NR. 4   TYPE:  HOMOGENEOUS PLATE
c$$$   MU :   1.00000    .00000 |    1.00000    .00000 |    1.00000    .00000
c$$$   EPS:   7.00000    .00000 |    9.00000    .00000 |    2.00000    .00000
c$$$   THE SAMPLE CONSISTS OF     32 UNIT SLICES
c$$$
c$$$             PRIMITIVE LATTICE VECTORS
c$$$             AR1 = (      1.0000       .0000)
c$$$             AR2 = (       .0000      1.0000)
c$$$             UNIT VECTORS IN RECIPROCAL SPACE:
c$$$             B1  = (       .0000      6.2832)
c$$$             B2  = (     -6.2832       .0000)
c$$$
c$$$   RECIPROCAL VECTORS     LENGTH
c$$$  1        0    0        .000000E+00
c$$$  2       -1    0        .628319E+01
c$$$  3        0   -1        .628319E+01
c$$$  4        1    0        .628319E+01
...
c$$$
c$$$
c$$$    FREQUENCY   TRANSMITTANCE  REFLECTANCE   ABSORBANCE
c$$$------------------------------------------------------------
c$$$   .100000E+01   .211294E+00   .788706E+00   .396350E-13
c$$$   .106250E+01   .940804E+00   .591957E-01   .938971E-13
c$$$   .112500E+01   .543944E+00   .456056E+00   .410227E-13
c$$$   .118750E+01   .722408E+00   .277592E+00  -.286438E-13
c$$$   .125000E+01   .718011E+00   .281989E+00   .129341E-12
c$$$   .131250E+01   .588369E+00   .411631E+00  -.695000E-13
c$$$   .137500E+01   .359722E-02   .996403E+00  -.949241E-13
c$$$   .143750E+01   .973861E+00   .261393E-01   .115515E-12
c$$$   .150000E+01   .544856E+00   .455144E+00   .320854E-13
c$$$   .156250E+01   .598532E+00   .401468E+00   .579536E-13
c$$$   .162500E+01   .974029E+00   .259715E-01   .124737E-12
c$$$   .168750E+01   .774661E+00   .225339E+00  -.990874E-14
c$$$   .175000E+01   .590934E+00   .409066E+00   .113687E-12
c$$$   .181250E+01   .183498E-13   .100000E+01  -.877076E-14
c$$$   .187500E+01   .781130E-12   .100000E+01  -.133227E-14
c$$$   .193750E+01   .880987E+00   .119013E+00  -.106859E-12
c$$$   .200000E+01   .815268E+00   .184732E+00   .779377E-13
c$$$CCCCCCCCC-------> HERE ENDS THE FIRST OUTPUT FILE (WRITTEN IN UNIT 6)
c$$$CCCCCCCCC-------> HERE STARTS THE SECOND OUTPUT FILE (WRITTEN IN UNIT 6)
c$$$     ****************************************************
c$$$     ************** OUTPUT: BAND STRUCTURE **************
c$$$     ****************************************************
c$$$   K_PARALLEL=     .000000     .000000
c$$$   COMPONENT NR. 1   TYPE:  PHOTONIC CRYSTAL 
c$$$   MU :   1.00000    .00000 |       1.00000    .00000      1.00000    .00000   
c$$$   EPS:   2.00000    .00000 |       5.00000    .00000      5.00000    .00000   
c$$$     S:                              .35000                 .35000             
...
c$$$
c$$$
c$$$ FREQUENCY       NORMALIZED K_Z
c$$$ ---------       --------------
c$$$ .1000E+01    .1121E+00  .1121E+00 -.1121E+00 -.1121E+00 
c$$$ .1063E+01   -.2791E+00  .2791E+00  .2791E+00 -.2791E+00 
c$$$ .1125E+01   -.4263E+00  .4263E+00  .4263E+00 -.4263E+00 
c$$$ .1188E+01    .5695E+00  .5695E+00 -.5695E+00 -.5695E+00 
c$$$ .1250E+01    .7115E+00  .7115E+00 -.7115E+00 -.7115E+00 
c$$$ .1313E+01    .8543E+00 -.8543E+00  .8543E+00 -.8543E+00 
c$$$ .1375E+01   -.1000E+01 -.1000E+01 
c$$$              .3456E-01 -.3456E-01 
c$$$ .1438E+01   -.8735E+00 -.8735E+00  .8735E+00  .8735E+00 
c$$$ .1500E+01   -.7292E+00 -.7292E+00  .7292E+00  .7292E+00 
c$$$ .1563E+01   -.5853E+00 -.5853E+00  .5853E+00  .5853E+00 
c$$$ .1625E+01   -.4388E+00 -.4388E+00  .4388E+00  .4388E+00 
c$$$ .1688E+01   -.2849E+00  .2849E+00 -.2849E+00  .2849E+00 
c$$$ .1750E+01    .9137E-01  .9137E-01 -.9137E-01 -.9137E-01 
c$$$ .1813E+01   -.5078E-10 -.2143E-10 
c$$$              .1608E+00 -.1608E+00 
c$$$ .1875E+01    .2938E-06 -.7726E-11 
c$$$              .1455E+00 -.1455E+00 
c$$$ .1938E+01    .1511E+00  .1511E+00 -.1511E+00 -.1511E+00 
c$$$ .2000E+01    .3269E+00  .3269E+00 -.3269E+00 -.3269E+00 
c$$$CCCCCCCCC-------> HERE ENDS THE SECOND OUTPUT FILE (WRITTEN IN UNIT 6)
c$$$CCCCCCCCC-------> THIS IS THE LAST LINE OF THE FILE
c$$$
c$$$                                                                            ****
