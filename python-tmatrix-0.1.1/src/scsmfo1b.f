...
c     code for the calculation of the efficiency factors and                 c
c     scattering matrix for an ensemble of external spheres.                 c
c                                                                            c
...
c       and horizontal coordinates.                                          c
c                                                                            c
c                                                                            c
...
c     scfodim.f, which contains the parameter dimensions:                  c
c                                                                            c
...
c     dmckwski@eng.auburn.edu                                                c
      ...
      subroutine tmnsfixed(npart,xi,xp,yp,zp,sni,ski,
     &                     xscale,rscale,rirscale,riiscale,
     &                     itermax,meth,eps,qeps1,qeps2,phi,theta,
     &                     qet,qat,qst,g,qei,qai,sa,sm,errcode)
...
c
     ...
     1       xp(npd),yp(npd),zp(npd),qai(npd,3),qei(npd,3)
      ...
      data ci/(0.0d0,1.0d0)/
      integer itermax,meth
      real*8 eps,qeps1,qeps2,phi,theta
C      data itermax,eps,rlx,qeps1,qeps2/100,1.0d-6,0,1.0d-3,1.0d-9/
C      data phi,theta/0.0d0,0.0d0/
      data itest/0/
...
C         write(*,'('' npart:'',$)')
C         read(*,*) npart
C         write(*,'('' xscale, rscale, rirscale, riiscale:'',$)')
C         read(*,*) xscale,rscale,rirscale,riiscale
C         write(*,'('' output file (return for none):'',$)')
C         read(*,'(a)') fout
C         if(fout.eq.'/') fout=' '
C         write(*,'('' amn output file (return for none):'',$)')
C         read(*,'(a)') famn
C         if(famn.eq.'/') famn=' '
C         write(*,'('' itermax, eps, meth, qeps1, qeps2:'',$)')
C         read(*,*) itermax,eps,meth,qeps1,qeps2
C         write(*,'('' number of fixed orientations:'',$)')
C         read(*,*) norien
C         do i=1,norien
C            write(*,'('' alpha, beta#'',i2,'':'',$)') i
C            read(*,*) ea(1,i),ea(2,i)
C         enddo
C         write(*,'('' nt, ipltopt:'',$)')
C         read(*,*) nt,ipltopt
C      else
...
C         read(1,*) npart
C         read(1,*) xscale,rscale,rirscale,riiscale
C         read(1,'(a)') fout
C         if(fout.eq.'/') fout=' '
C         fout=fout(:index(fout,' '))
C         read(1,'(a)') famn
C         if(famn.eq.'/') famn=' '
C         famn=famn(:index(famn,' '))
C         read(1,*) itermax,eps,meth,qeps1,qeps2
C         read(1,*) norien
C         do i=1,norien
C            read(1,*) ea(1,i),ea(2,i)
C         enddo
C         read(1,*) nt,ipltopt
C         if(fpos.ne.' ') then
C            close(1)
...
C      endif
      ...
      zm=0.
      do i=1,npart
C         if(fdat.eq.'con'.and.fpos.eq.' ') then
C            write(*,'('' x, xp, yp, zp, Re(m), Im(m)#'',i2,'':'',$)') i
C            read(*,*) xii,xpi,ypi,zpi,rir,rii
C         else
C            read(1,*,end=25,err=25) xii,xpi,ypi,zpi,rir,rii
C         endif
         xi(i)=xscale*xi(i)
         xp(i)=xscale*rscale*xp(i)
         yp(i)=xscale*rscale*yp(i)
         zp(i)=xscale*rscale*zp(i)
...
C      if(fout.ne.' ') then
C         open(1,file=fout)
C         write(1,'('' scsmfo results'')')
C         if(fdat.eq.'con') then
C            write(1,'('' position file:'',a)') fpos
C            write(1,'('' number of spheres:'',i6)') npart
C            write(1,'('' xscale,rscale,riscales'',4e12.4)') xscale,
C     *           rscale,rirscale,riiscale
C         else
C            write(1,'('' input file:'',a)') fdat
C         endif
C         write(1,'('' xv:'', e12.4)') xv
C         close(1)
C      endif
...
C     I (Ovidio) added the next three lines (26/02/2010)
      norien=1
      ea(1,1)=phi
      ea(2,1)=theta
...
C         write(*,*)
C         write(*,'('' solving for alpha,beta:'',2f8.1)')
C     1        ea(1,io),ea(2,io)
         niter=itermax
...
c
         ...
         qst=qet-qat
         qet=(qet0+qetpi2)/2.
C        I (Ovidio) added the next line (26/02/2010)
         g=dble(gmn(1))/3.

C         write(*,'(''              qe(0)      qe(pi/2)    qe(pi/4)'',
C     *             ''     qa(0)      qa(pi/2)    qa(pi/4)'')') 
C         write(*,'('' sphere : '',6e12.4)')
C     *    qet0,qetpi2,qetpi4,qat0,qatpi2,qatpi4
C         write(*,'('' cluster: '',6e12.4)')
C     *    qetc0,qetcpi2,qetcpi4,qatc0,qatcpi2,qatcpi4
C         write(*,'('' <cos theta>:'',e12.4)') dble(gmn(1))/3.

C         if(fout.ne.' ')  then
C            open(1,file=fout,access='append')
C            write(1,'('' alpha,beta:'')')
C            write(1,'(2f8.1)') ea(1,io),ea(2,io)
C            write(1,'('' ave. cluster qext,qabs,qsca,<cos theta>:'')')
C            write(1,'(4e13.5)') qet,qat,qst,dble(gmn(1))/3.0d0
C            write(1,'('' cluster qe(0),qe(pi/2),qe(pi/4),'',
C     *             ''qa(0),qa(pi/2),qa(pi/4)'')') 
C            write(1,'(6e13.5)') qet0,qetpi2,qetpi4,qat0,qatpi2,qatpi4
C            write(1,'('' sphere qext and qabs:'')')
C            write(1,'('' sphere   qe(0)       qe(pi/2)     qe(pi/4)'',
C     *             ''      qa(0)       qa(pi/2)     qa(pi/4)'')') 
C            do i=1,npart
C               write(1,'(i5, 6e13.5)') i, qei(i,1),qei(i,2),
C     *          qei(i,3),qai(i,1),qai(i,2),qai(i,3)
C            enddo
C            write(1,'('' scattering map grid size, grid option:'')')
C            write(1,*) nt,ipltopt
C         endif
...
C         if(famn.ne.' ') then
C            open(2,file=famn,access='append')
C            write(2,'(i5,f10.3,2f8.2)') nodrt,xv,ea(1,io),ea(2,io)
C            do ik=1,2
C               do n=1,nodrt
C                  nn1=n*(n+1)
C                  do m=-n,n
C                     mn=nn1+m
C                     do ip=1,2
C                        write(2,'(2e13.5)') amn0(ip,mn,ik)
C                     enddo
C                  enddo
C               enddo
C            enddo
...
C         if(ipltopt.eq.1.and.nt.gt.0) then

C            do idir=1,-1,-2
C               if(idir.eq.1) then
C                  write(*,'('' calculating forward scattering map'')')
C                  if(fout.ne.' ') then
C                     write(1,'('' forward scattering'')')
C                     write(1,'(''  S11         S22         S33'',
C     *     ''         S44         S21         S32         S43'',
C     *     ''         S31         S42         S41         S12'',
C     *     ''         S23         S34         S13         S24'',
C     *     ''         S14'')')
C                  endif
C               else
C                  write(*,'('' calculating backward scattering map'')')
C                  if(fout.ne.' ') then
C                     write(1,'('' backward scattering'')')
C                     write(1,'(''  S11         S22         S33'',
C     *     ''         S44         S21         S32         S43'',
C     *     ''         S31         S42         S41         S12'',
C     *     ''         S23         S34         S13         S24'',
...
C                     r=sqrt(x*x+y*y)
C                     if(r.le.1.) then
C                        ct=sqrt((1.-r)*(1.+r))*idir
C                        th=dacos(ct)*180./pi
C                        if(x.eq.0..and.y.eq.0.) then
C                           phi=0.
C                        else
C                           phi=datan2(x,y)
C                        endif
C                        call scatfunc(amn0,nodrt,xv,ct,phi,sa,sm)
C                        sphi=sin(phi)
C                        cphi=cos(phi)
C                     else
C                        do i=1,4
...
C                        enddo
C                     endif
C                     if(fout.ne.' ') write(1,'(16e12.4)')
C     *                  (sm(i,i),i=1,4),
C     *                  (sm(i+1,i),i=1,3),(sm(i+2,i),i=1,2),sm(4,1),
C     *                  (sm(i,i+1),i=1,3),(sm(i,i+2),i=1,2),sm(1,4)
C                  enddo
C               enddo
C            enddo
C         elseif(ipltopt.eq.2.and.nt.gt.0) then
C            write(*,'('' calculating scattering map'')')
C            if(fout.ne.' ') then
C               write(1,'('' scattering map'')')
C               write(1,'(''  S11         S22         S33'',
C     *     ''         S44         S21         S32         S43'',
C     *     ''         S31         S42         S41         S12'',
C     *     ''         S23         S34         S13         S24'',
...
C               ct=cos(th)
C               do iphi=0,nt
C                  phi=2.*pi*iphi/dble(nt+1)
C                  call scatfunc(amn0,nodrt,xv,ct,phi,sa,sm)
C                  if(fout.ne.' ') write(1,'(16e12.4)') (sm(i,i),i=1,4),
C     *               (sm(i+1,i),i=1,3),(sm(i+2,i),i=1,2),sm(4,1),
C     *               (sm(i,i+1),i=1,3),(sm(i,i+2),i=1,2),sm(1,4)
C               enddo
C            enddo
C         endif
C         if(fout.ne.' ') close(1)
      enddo
...
C         write(*,'(''+order:'',i5,$)') n
         ...
         do l=l1,l2
            ll1=l*(l+1)
            c=fnr(n+n+1)*fnr(l+l+1)*dcmplx(0.0d0,1.0d0)**(l-n)
            w2=min(n+l,nodrg)
     ...
     *                  +amn0(ip,mn,2)*conjg(amn0(iq,kl,2)))
      ...
      do w=1,nodrg
         ww1=(w*(w+1))/2
         gmn(ww1)=dcmplx(dble(gmn(ww1)),0.0d0)/g0
         do u=1,w
            uw=ww1+u
            gmn(uw)=(-1)**u*2.0d0*gmn(uw)/g0
         enddo
      enddo
...
c
      ...
      do i=1,4
c         WRITE (*,5001) dreal(sa(i)),dimag(sa(i))
         do j=1,4
            sp(i,j)=sa(i)*conjg(sa(j))*.5/xv/xv
         enddo
      enddo
c 5001 FORMAT (2E12.4)
...
c
     ...
     1             qs1,an1(1,1,i))
      ...
      enddo
C      write(*,'('' single sphere max. order:'',i5)') nodrmax
...
C      write(*,'('' estimated cluster expansion order:'',i5)') nodrtmax
      nblktmax=nodrtmax*(nodrtmax+2)
c
      do i=1,npart
C         write(*,'(''+assembling interaction matrix row:'',i4,$)') i
         do j=i+1,npart
            ij=.5*(j-1)*(j-2)+j-i
            x=xp(i)-xp(j)
            y=yp(i)-yp(j)
...
15    do n=1,nblktmax
      ...
      do k=1,2
C         write(*,'('' solving for incident state '',i1)') k
      ...
      do i=1,npart
         ...
         do n=1,nodr(i)
            ...
            do m=-n,n
               mn=nn1+m
               do ip=1,2
                  cn=dble(1.0d0/an1(ip,n,i)-1.0d0)
                  qai(i,1)=qai(i,1)
...
C      write(*,'('' cluster expansion order:'',i5)') nodrtmax
...
c              enorm=enorm+anp(ip,n,i)*conjg(anp(ip,n,i))
      ...
      enddo
      err=err/enorm
C      write(*,'(''+iteration:'',i4,'' error:'',e13.5,$)') iter,err
      if(err.lt. eps) then
C         write(*,*)
         return
      endif
...
c
   ...
   20 continue
      xi(n)=dns
      xi(n-1)=dble(n)/ds-1.0d0/(dns+dble(n)/ds)
      do 25 i=n-2,1,-1
         sn=dble(i+1)/ds
         xi(i)=sn-1.0d0/(xi(i+1)+sn)
   25 continue
c
...
c
   ...
   20 continue
      psi(n)=dns
      psi(n-1)=dble(n)/ds-1.0d0/(dns+dble(n)/ds)
      do 25 i=n-2,1,-1
         sn=dble(i+1)/ds
         psi(i)=sn-1.0d0/(psi(i+1)+sn)
   25 continue
c
c forward recurrence: psi(n) computed from dn1(n)
c
...
c
      ...
      do 20 i=ns-1,n,-1
         sn=dble(i+1)/ds
         psins=sn-1.0d0/(psins+sn)
   20 continue
      psi(n)=psins
      psi(n-1)=n/ds-1.0d0/(psins+n/ds)
      do 25 i=n-2,1,-1
         sn=dble(i+1)/ds
         psi(i)=sn-1.0d0/(psi(i+1)+sn)
   25 continue
