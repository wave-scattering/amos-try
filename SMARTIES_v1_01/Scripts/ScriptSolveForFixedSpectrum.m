...
% <<../fig/schematicp.png>>
% 
c = 90; % in nm, i.e. 30 x 90nm full-axes
a = 110; % in nm
lambda = (550:5:1000).'; % in nm
points = length(lambda);
epsilon2 = ones(points,1).*(4^2); 
%epsilon2 = epsSi(lambda);
epsilon1 = 1.0^2; % for water
...
plot(lambda,[stC.Cext/pi/(a^2*c)^(2/3)]);
legend({['Cext (err. ', num2str(relerrExt,3),')'], ...
    ...
    'Location','Best');
title(['a=', num2str(a), ', c=',num2str(c),', dir ',sIncType,', N=', int2str(N), ', Nt=', int2str(nNbTheta)]);
